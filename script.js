    ...
    if (!gameOver && board[row][col] === "") {
        ...
        if (checkWin()) {
            document.getElementById(
                "twoPlayersWinnerText",
            ).textContent = `${currentPlayer} wins!`;
            gameOver = true;
        } else if (checkTie()) {
            document.getElementById("twoPlayersWinnerText").textContent =
                "It's a tie!";
            gameOver = true;
        } else {
...
function vsComMakeMove(row, col) {
    if (!gameOver && board[row][col] === "") {
        ...
        if (checkWin()) {
            document.getElementById(
                "vsComputerWinnerText",
            ).textContent = `${currentPlayer} wins!`;
            gameOver = true;
        } else if (checkTie()) {
            document.getElementById("vsComputerWinnerText").textContent =
                "It's a tie!";
            gameOver = true;
        } else {
            currentPlayer = currentPlayer === "X" ? "O" : "X";
